<template>
  <div class="relative">
    ...
    <div ref="target">
      ...
      <div
        class="w-full max-h-[190px] bg-black-light text-base z-50 list-none border border-gray-dark divide-y divide-gray-100 rounded shadow my-2 absolute overflow-y-auto"
        ...
        <ul class="py-1">
          <li v-for="option in props.options" :key="option[keyField]">
            <div class="text-sm hover:bg-gray-dark text-white block px-4 py-2 cursor-pointer"
              :class="{ 'bg-gray-dark': option[keyField] === selectedValue }" @click="selectOption(option[keyField])">{{
                option[valueField] }}
            </div>
          </li>
        </ul>
