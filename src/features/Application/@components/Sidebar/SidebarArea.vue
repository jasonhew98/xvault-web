<template>
  <aside class="sidebar lg:relative absolute lg:translate-x-0 -translate-x-full duration-300 ease-linear bg-black"
    ref="target" :class="{
    ...
    <div class="h-full w-full flex flex-col border-r border-gray-dark">
      <div class="mb-20 mr-4 flex flex-col items-center text-center">
        <div class="mb-3 rounded-full bg-black-light p-1.5 cursor-pointer" @click="goToProfile">
          <svg width="30" height="35" viewBox="0 0 515 595" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="sidebar__menu">
        <div v-for="menuItem in topMenuItems" :key="menuItem.id">
          <div class="sidebar__description">
            <div
              class="inline-flex items-center cursor-pointer text-gray-light hover:text-white transition-colors duration-200 ease-in-out"
              ...
              <span class="text-base font-medium" :class="{ 'text-white': isCurrentSelected(menuItem.id) }">
                {{ menuItem.name }}
              </span>
            </div>
          </div>
      ...
      <div class="sidebar__menu">
        <div v-for="menuItem in bottomMenuItems" :key="menuItem.id">
          <div class="sidebar__description">
            <div
              class="inline-flex items-center cursor-pointer text-gray-light hover:text-white transition-colors duration-200 ease-in-out"
              @click="setSelectedMenuItem(menuItem.id)">
